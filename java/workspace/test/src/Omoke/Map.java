...
public class Map {
	...
	private final short BLACK = 1;//black stone values is 1

	private final short WHITE = -1;// white stone values is -1
	...
	public Map(MapSize ms) {//call getSize method values is 20
		...
		map = new short[ms.getSize()][];//map reset data [20][] 

		for (int i = 0; i < map.length; i++)//for loop reiteration map.length

			map[i] = new short[ms.getSize()];// map[i] instance for short array value is 20
	...
	public short getXY(int y, int x) {
		...
		return map[y][x];//get value 

	}

	public boolean getCheck() {

		return checkBNW;//return current stone
	...
	public void changeCheck() {
		...
		if (checkBNW)
			...
			checkBNW = false;//true is false

		else

			checkBNW = true;//false is true 
