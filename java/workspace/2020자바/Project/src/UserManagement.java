...
public class UserManagement extends BaseFrame {
	...
	public UserManagement() {
		super(800, 500, "유저관리", new BorderLayout(), 2);
		setDesign();
		setAction();
		setVisible(true);
	}
						...
						for (int j = 0; j < count; j++) {
							for (int k = 0; k < 6; k++) {
								if (((JTextField) arr1.get(j)[k]).getText().equals("")) {
									try {
										stmt.execute("delete from user where u_no = '" + arr1.get(j)[6] + "'");
										eMsg("빈칸이 있는 " + arr1.get(j)[6] + "번 유저를 삭제하였습니다.");
									...
									try {
										stmt.execute(
												"update user set u_name = '" + ((JTextField) arr1.get(j)[0]).getText()
